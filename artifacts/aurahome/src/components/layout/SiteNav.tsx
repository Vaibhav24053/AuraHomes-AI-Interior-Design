      ...
      <nav className="mx-auto flex max-w-[1240px] items-center justify-between rounded-full border border-[#d7cbbb]/80 bg-[#f3ecdf]/85 px-4 py-3 shadow-[0_10px_40px_rgba(74,52,32,.06)] backdrop-blur-md sm:px-6" aria-label="Primary navigation">
        ...
        <div className="hidden items-center gap-7 lg:flex">
          {items.map(([label, href]) => (
            <Link key={href} href={href} className="nav-link text-[12px] font-medium text-[#536059] transition-colors hover:text-[#b8573b]" data-testid={`link-nav-${label.toLowerCase().replaceAll(' ', '-')}`}>
              {label}
            </Link>
          ))}
        </div>
        <div className="hidden items-center gap-4 sm:flex">
          <Link href="/design" className="group flex items-center gap-2 rounded-full bg-[#29352f] px-4 py-2.5 text-[12px] font-semibold text-[#f3ecdf] transition-transform hover:-translate-y-0.5 btn-primary-glow" data-testid="link-nav-get-started">
            Start Designing <ArrowUpRight size={14} className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </Link>
        </div>
      ...
      {open && (
        <div className="mx-0 mt-2 rounded-3xl border border-[#d7cbbb] bg-[#f3ecdf] p-5 shadow-xl sm:hidden">
          <div className="grid gap-4">
            ...
            <Link href="/design" className="mt-1 flex items-center justify-center rounded-full bg-[#b8573b] px-4 py-3 text-sm font-semibold text-[#f3ecdf]" data-testid="link-mobile-get-started">
              Start Designing
            </Link>
          </div>
        </div>
